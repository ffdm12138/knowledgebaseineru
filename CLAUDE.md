...
| POST | `/write/jobs/{job_id}/match-catalog` | 目录匹配 → 候选文献 + prompt |
| POST | `/write/jobs/{job_id}/deep-read` | 全文精读。Body: `{"paper_ids"}` |
| POST | `/write/jobs/{job_id}/build-story` | 生成故事线 + prompt |
| POST | `/write/jobs/{job_id}/build-tex` | 生成 LaTeX 项目 + references.bib |
| POST | `/write/jobs/{job_id}/copy-figures` | 复制被引用图到 figures/ |
| POST | `/write/jobs/{job_id}/validate` | 校验产物 + `\cite` 一致性 |
| GET | `/status` | 系统状态：端口、模式、文献统计、目录条目数 |

所有 `/prompt/*` 与 `/write/jobs/*` **不调用 LLM**，只生成可复制的 prompt 文本与结构化模板文件。
...
博士论文级综述写作，流程：**建任务 → 目录匹配 → 全文精读 → 故事线 → LaTeX 项目 → 图片 → 校验**。所有 LLM 步骤只生成 prompt（写到 `write/<job>/logs/prompts/`），由人手动跑大模型后回填对应文件。规则见 [skills/literature_review_writer/CLAUDE.md](skills/literature_review_writer/CLAUDE.md)。
...
python scripts/write_review.py match     --job 001_风吹雪升华参数化的发展脉络
# 用大模型跑 logs/prompts/01_catalog_matching_prompt.md，把入选文献回填 planning/selected_papers.json
python scripts/write_review.py deep-read --job 001_风吹雪升华参数化的发展脉络 \
    --papers 1999_dery_吹雪体相模型 2025_huang_雪粒破碎促进升华
python scripts/write_review.py story     --job 001_风吹雪升华参数化的发展脉络
python scripts/write_review.py tex       --job 001_风吹雪升华参数化的发展脉络
python scripts/write_review.py figures   --job 001_风吹雪升华参数化的发展脉络
python scripts/write_review.py validate  --job 001_风吹雪升华参数化的发展脉络
```
也可走 `/write/jobs*` API 或 `--input doc.md` 用项目文档创建任务。

### BibTeX 系统
- `literature_catalog.json` 每篇含 `citation.{bib_key,bibtex,citation_style_name,source,verified}`，13 篇已补全。
- `data/catalog/references.bib` 全库汇总，由 `scripts/sync_bib_from_catalog.py` 从 catalog 同步。
- `scripts/validate_bib.py` 校验 citation 与 references.bib 一致性（bib_key 唯一、bibtex 含 title/author/year、DOI 已写入）。
- 写作时 `tex_project.py` 自动按 `selected_papers` 从全局 bib 抽取生成 `write/<job>/tex/references.bib`。
...
- 引用图必须从 `data/papers/<pid>/images/` 复制到 `write/<job>/figures/<pid>/` 并记 README，TeX 用相对路径 `../figures/<pid>/<img>`。
- 禁止编造文献、图号、公式、DOI。
- introduction / method 按博士论文标准（见 `skills/literature_review_writer/templates/`）。

### Tex 项目解耦（可整体挪走）
每个 `write/<job>/tex/` 是**自包含、可独立复制/挪走**的 LaTeX 项目：
- `tex/references.bib` 是任务专属副本，由 `tex_project.py` 从全局 `data/catalog/references.bib` 按 `selected_papers` 抽取生成，**不依赖**全局 bib 或 catalog。挪走后仍可编译。
...
9. **写作 skill 的 LLM 步骤需手动跑**。`/write/jobs/*` 与 `scripts/write_review.py` 各步生成 prompt + 模板文件，但 introduction/method 正文、paper_notes 内容、selected_papers 裁剪都需人用大模型跑 prompt 后回填。校验对"未引用 bib 条目"仅作警告（模板阶段正常），对"`\cite` 找不到"作硬错误。
10. **figure_manager 默认复制全部候选图**。`copy-figures` 当前从 `figure_candidates.md` 复制所有图到 `figures/`，实际只在 TeX 中 `\includegraphics` 的才被引用；多余图无害但占空间，可手动删。后续可改为只复制 TeX 引用的图。但**进 TeX 的图必然已复制到 `figures/`**（portability_check 会拦截指向 `data/papers` 的外部引用），保证 tex 项目可挪走。
