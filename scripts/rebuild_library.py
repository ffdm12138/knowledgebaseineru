...
_METHOD_DIRS = ["hybrid_auto", "hybrid_txt", "hybrid_ocr", "auto", "txt", "ocr"]


def find_legacy_output(stem: str) -> Path | None:
    """在 data/parsed/<stem>/ 下找到含 .md 的输出目录"""
...
def reconvert(f: Path, paper_id: str, backend: str, method: str, effort: str,
    ...
    cmd = [converter._get_env() and "mineru", "-p", str(f), "-o", str(tmp_out),
...
def process_one(f: Path, reconvert_flag: bool, backend: str, method: str,
    ...
    if not reconvert_flag:
        source_dir = find_legacy_output(stem)
        if source_dir:
            logger.info(f"  复用旧输出: {source_dir}")
